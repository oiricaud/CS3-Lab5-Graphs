...
import java.util.Stack;
	// DEPTH First SEARCH
	// This class basically looks at the first vertex that is adjacent to the current node
	// then it follows this vertex and goes to that specific row. It continues doing this until 
...
public class DepthSearch {
	...
	public static void calculator(Object[][] adjacencyMatrix, LinkedList listVertices, LinkedList<String> listofEdges,
		...
		System.out.println("listvertices.size() " + listVertices.size());
		while(rowCount < listVertices.size()){
		//	int distance =  (Integer) adjacencyMatrix[rowCount+1][rowCount+1];
			// Start at any vertex 
	...
	private static LinkedList<Object> adjacentVertices(Object[][] adjacencyMatrix, Object vertex, graphClass graph, int rowCount, int distance) {
		...
		while(visitEdge < adjacencyMatrix.length){
			if((Integer) adjacencyMatrix[rowCount+1][visitEdge] >= 1 && graph.getColor(visitEdge-1).equals("white")){
				...
				//distance++;
				//graph.getDistance(visitEdge-1);
				System.out.println("HEREH " + graph.getVertex(visitEdge-1));
				System.out.println("column: " + column);
				System.out.println("visitedge: " + visitEdge);
				System.out.println("DISTANCE: " + distance);
				if(column != adjacencyMatrix.length-1){
				adjacentVertices(adjacencyMatrix,  graph.getVertex(visitEdge-1), graph, column, distance);
				//System.out.println("distance: " + graph.getDistance(rowCount+1));
				//column=1;
				}
				else{
					column=0;
				}
				//visitEdge= adjacencyMatrix.length;
			}
			else{
